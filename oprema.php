<?php
    ...
    echo"<link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/animate.css/4.0.0/animate.min.css'/>
        ...
        <div class='animate__animated animate__bounceInLeft bar2'>
            <h2><a href='index.php' class='bar'><i class='icon-home'></i> Početna</a> <span> / Oprema</span></h2>
            <br></br>
    ...
    if(isset($_GET['obrisi'])){
        ...
        $query = $konekcija -> prepare('DELETE FROM oprema WHERE id=?');
...
<div class="container">
                    ...
                    <form action="" method="GET" class="filter">
                        <?php
                            ...
                            if($filter_query_run)
                            {
                                ...
                                ?>
                                <button type="submit" class="btn btn-success filter_button">Traži</button>
                                <a href='oprema.php' class="btn btn-primary refresh_btn">Osvježi</a>
                                <?php
                            }else{
    ...
    <?php
if(Admin()){
echo"<a class='btn novododaj animate__animated animate__bounceInLeft' href='dodaj_opremu.php'><i class='icon-plus'></i><b> Dodaj proizvod</b></a>";
}
echo"<a class='btn osvjezi animate__animated animate__bounceInLeft' href='oprema.php'><i class='icon-refresh'></i><b> Osvježi</b></a>";
            ...
            if(isset($_GET['zivotinje'])){
                ...
                foreach($zivotinjachecked as $rowzivotinja){
                    ...
                    $proizvodi = "SELECT*FROM oprema WHERE kategorija_id IN ($rowzivotinja) LIMIT " . $ova_stranica_rezultati . ',' .  $brojRezultataPoStranici;
                                            ...
                                            <form action="" method="post">
                                                <div class="box_proizvod mySlides animate__animated animate__pulse">
                                                    <?php
                                                        ...
                                                        echo"
                                                            <a href='detalji_oprema.php?detalji=$row[id]' class='za_detalje'>
                                                                <img class='slika' src='sve_slike/" . $row['slika'] . "' style='width: 200px;'/>
                                                                <h4 class='cijena_proizvod'>" . $row['cijena'] . " kn" ."</h4>
                                                            ...
                                                            if(Admin()){
                                                                ...
                                                                echo"
                                                                    <a href='uredi_oprema.php?edit=$row[id]' class='btn-edit'><i class='icon-edit'></i></a>
                                                                    <a href='urediSlikuOpreme.php?edit=$row[id]' class='btn-edit2'><i class='icon-picture'></i></a>
                                                                    <a href='oprema.php?obrisi=$row[id]' onclick=\"javascript: return confirm('Jeste li sigurni da želite obrisati opremu?');\" class='btn-delete'><i class='icon-trash'></i> </a>
                                                                ";
                                                            }elseif(User()){
                                                                if($row['dostupnost'] != 'Samo u poslovnici'){
                                                                    echo"<input type='submit' class='btn btn-primary cart' value='U košaricu' name='add_to_cart'><br></br>";
                ...
                $query='SELECT * FROM oprema LIMIT ' . $ova_stranica_rezultati . ',' .  $brojRezultataPoStranici;
                $rezultat = mysqli_query($konekcija, $query);
                ?>
                    <section class="proizvod">
                        <?php if(Admin()){
                            echo"<a class='btn pdf2 animate__animated animate__bounceInLeft' href='pdf_oprema.php'><i class='icon-print'></i><b> Kreiraj PDF</b></a>";
                        } ?>
                ...
                <?php
                    //PAGINACIJA
                        ...
                        if ($brojStranica>1) {
                            ...
                            if($stranica>1) {
                                echo "<li class='page-item'><a class='page-link' href='oprema.php?stranica=" . ($stranica-1) . "'>
                                &laquo Prethodna </a></li>";
                            }
                            for($i=1;$i<=$brojStranica; $i++){
                                if ($i==$stranica) echo "<li class='page-item active'><span class='page-link'> $i </span></li>";
                                else echo "<li class='page-item class='page-link'><a class='page-link' href='oprema.php?stranica=$i'> $i </a></li>";
                            }
