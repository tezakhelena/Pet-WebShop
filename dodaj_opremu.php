<?php 
    ...
    <div class='animate__animated animate__bounceInLeft bar2'>
        <h2><a href='index.php' class='bar'><i class='icon-home'></i> Početna</a> <span> / </span> <a href='oprema.php' class='bar'> Oprema</a><span> / Dodaj novu opremu</span></h2>
        <br>
                        ...
                        if(in_array($_FILES['slika']['type'], $dozvoljeni_MIME)){
                            if($kategorija_id != 9){
                                if($dostupnost != "Neodabrano"){
                                    if($opis > 2){
                                        $query = $konekcija->prepare('INSERT INTO oprema(naziv, cijena, slika, kategorija_id, dostupnost, opis) VALUES (?,?,?,?,?,?)');
                                        $query->bind_param('sdsdss', $naziv, $cijena, $slika, $kategorija_id, $dostupnost, $opis);
                                        if ($query->execute()) {
                                            move_uploaded_file($slika_tmp_name, $slika_folder);
                                            echo '<div class="alert alert-success">Uspješno ste dodali opremu!</div>';
                                        }else{
...
<div class="container">
    <div class="dodavanje animate__animated animate__pulse">
        <form action="<?php $_SERVER['PHP_SELF'] ?>" method="post" enctype="multipart/form-data">
            <h3>Dodaj novu opremu u bazu</h3>
                ...
                <input type="text" value="<?= isset($naziv)? $naziv:'' ?>" placeholder="Unesi naziv opreme" name="naziv" class="box">
                ...
                <?= isset($input_error['opis'])? '<label class="text-danger">'.$input_error['opis'].'</label><br>':'';  ?>            </br>
                <button type="submit" class="btn btn-success" onclick="return javascript(location.refresh(true))" name="dodajOpremu"><i class="icon-save"></i> Spremi</button>
        </form>
    </div>

    <div>
        <img src='sve_slike/oprema1.png' class='slika1 animate__animated animate__pulse'  />
    </div>
</div>
