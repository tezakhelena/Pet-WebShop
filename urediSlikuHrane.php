<?php
    $title = "Uređivanje fotografije hrane - Pet Shop"; 
    ...
    <div class='animate__animated animate__bounceInLeft bar2'>
        <h2><a href='index.php' class='bar'><i class='icon-home'></i> Početna</a> <span> / </span> <a href='hrana.php' class='bar'> Hrana</a><span> / <i class='icon-picture'></i>  Mjenjanje fotografije</span></h2>
        <br>
    ...
    if(isset($_POST['urediHranu'])){
        ...
        if(empty($slika)){
            ...
            $update = "UPDATE hrana SET slika='$slika' WHERE id = $id";
            $upload = mysqli_query($konekcija, $update);
            if($upload){
                move_uploaded_file($slika_tmp_name, $slika_folder);
                echo '<div class="alert alert-success animate__animated animate__backInLeft">Fotografija je uspješno promjenjena!</div>';
            } else{
...
<div class="container">
        ...
        <?php
        $query="SELECT*FROM hrana WHERE id = $id";
        ...
        <h2 class="trenutna_foto">Trenutna fotografija: </h2></br>
            ...
            <form action="<?php $_SERVER['PHP_SELF'] ?>" method = "post" enctype = "multipart/form-data" class="form_uredi">
                ...
                <input type="submit" class="btn btn-success uredi_slikugumb" name="urediHranu" value="Spremi promjene">
                <?php
