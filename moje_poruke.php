<?php $title = "Inbox - Pet Shop"; 
    ...
    if(isset($_SESSION['korisnik_id'])){
        ...
        <div class='animate__animated animate__bounceInLeft bar2'>
            <h2><a href='index.php' class='bar'><i class='icon-home'></i> Početna</a> <span> / <i class='icon-inbox'></i> Moje poruke</span></h2><br>
            <br>
            <a class='btn btn-danger px-3 me-2' href='index.php' ><i class='icon-arrow-left'></i> Povratak </a>
            <a href='poruke.php' class='btn btn-primary'>Pošalji novo pitanje</a><br><br><br>
        ";        
        ...
        $query="SELECT * FROM poruke WHERE korisnik_id='$korisnik_id'";
        ...
        $query="SELECT*FROM poruke WHERE korisnik_id='$korisnik_id' ORDER BY poruke_id ASC LIMIT " . $ova_stranica_rezultati . " , " . $brojRezultataPoStranici;
        ...
        if(mysqli_num_rows($redci) > 0){
                        ...
                        <a class='link' href='detalji_poruka.php?detalji=$row[poruke_id]'>
                            <div class='card10' style='width: 100%;'>
                                <div class='upiti_box'>
                                    ...
                                    ";
                                    if($row['status_poruke'] == 1){
                                        echo '<p><a href="status_poruke.php?poruke_id='.$row['poruke_id'].'&status_poruke=2" class="btn btn-success"> Označi kao pročitano <i class="icon-ok"></i></a></p>';
                                    }
                                    echo"
                                    <a href='moje_poruke.php?obrisi=$row[poruke_id]' onclick=\"javascript: return confirm('Jeste li sigurni da želite izbrisati svoje pitanje?');\" class='btn-delete-poruka'><i class='icon-trash'></i> </a>
                                    </div>
                            </div></br>
                        </a><hr>";
            ...
            <div class='alert alert-warning animate__animated animate__backInLeft'>Nemate poruke s administratorom!</div>
            </div>";
        }
        $konekcija->close();
        //paginacija
        if ($brojStranica>1) {
            ...
            if($stranica>1) {
                echo "<li class='page-item'><a class='page-link' href='moje_poruke.php?stranica=" . ($stranica-1) . "'>
                &laquo Prethodna </a></li>";
            }
            for($i=1;$i<=$brojStranica; $i++){
                if ($i==$stranica) echo "<li class='page-item active'><span class='page-link'> $i </span></li>";
                else echo "<li class='page-item class='page-link'><a class='page-link' href='moje_poruke.php?stranica=$i'> $i </a></li>";
            }
